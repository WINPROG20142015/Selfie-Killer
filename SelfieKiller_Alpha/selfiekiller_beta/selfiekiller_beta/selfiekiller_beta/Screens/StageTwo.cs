...
{
    public class StageTwo : GameScreen
    {
        #region STAGE1 VARS
        ...
        //Newtral
        List<Newtral> loli = new List<Newtral>();
        float lolispawn = 0;
        bool isNewtralHit = false;

        //Flag
        Flag flag;
        float endPositionX = 21000.0f;

        //timer
        int counter = 1;
        int limit = 1;
        float countDuration = .25f; //every  2s.
        ...
        #endregion
        #region CONSTRUCTOR
        public StageTwo(ContentManager content)
        {
        ...
        public override void LoadContent()
        {
            ...
            backgrounds[0] = new Background(content, "Backgrounds/Background3", 0.2f);
            ...
            //enemyhit
            jejehit = content.Load<Texture2D>("Sprites/duckhit");
        ...
        public override void Update(GameTime gameTime, bool covered)
        {
            ...
            if (!gameEnd)
            {
                ...
                lolispawn += (float)gameTime.ElapsedGameTime.TotalSeconds;
                foreach (Newtral newtral in loli)
                {
                    newtral.Update();
                }

                if (timeLinedec < 250)
                { LoadEnemies(); LoadNewtrals(); }
                flag.Update();

                //LoseScreen
                if (player.life == 0)
                {
                    ScreenManager.AddScreen(new WinScreen(content, "stateTwo"));
                    gameEnd = true;
                }
        ...
        public void HandleCollision()
        {
            ...
            //Player's Attack Rect Touching and Newtral
            foreach (Newtral newtral in loli)
            {
                if (player.localRect.Intersects(newtral.localRect) && isPlayerHit == false
                    && player.isDestroy == true  &&
                    isNewtralHit == false
                    )
                {
                    for (int i = 0; i < loli.Count; i++)
                    {
                        ...
                        isNewtralHit = true;
                        gameEnd = true;
                        ScreenManager.AddScreen(new LoseScreen(content,"stateTwo"));
            ...
            if (CollisionOn == true)
            {
                foreach (Newtral newtral in loli)
                {
                    Trace.WriteLine(newtral.localRect.X);
                    if (player.localRect.Intersects(newtral.localRect) && isPlayerHit == false
                        && player.isDestroy == false && player.isAvoid == false)
                    {
                        isPlayerHit = true;
        ...
        public void LoadEnemies()
        {
            ...
            if (spawn >= 2)
            {
                ...
                if (jejemon.Count() < 4)
                {
                    jejemon.Add(new Enemy(content, new Vector2(2100, 270), "Sprites/duck"));
                }
            }
         ...
         public void LoadNewtrals()
        {
            ...
            if (lolispawn >= 1)
            {
                ...
                if (loli.Count() < 1)
                {
                    loli.Add(new Newtral(content, new Vector2(2100, 270), "Sprites/loli"));
                }
            }
            ...
            for (int i = 0; i < loli.Count; i++)
            {
                ...
                if (!loli[i].isVisible)
                {
                    ...
                    isPlayerHit = false;
                    if (loli[i].position.X < 0 - loli[i].texture.Width)
                    {
                        loli.RemoveAt(i);
                        i--;
                        CollisionOn = true;
